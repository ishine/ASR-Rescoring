...
def parse_json(
    ...
    if "all" in requirements:
        ...
        for utt_count, (utt_id, utt_content) in enumerate(json_data.items(), 1):
            ...
            output["all"][utt_id]["hyp"] = {
                hyp_id: hyp_content
                for index, (hyp_id, hyp_content) in enumerate(utt_content["hyp"].items()) if  index < n_best
            }
    ...
    if "hyp_text" in requirements:
        hyp_text = [
            [hyp["text"] for index, hyp in enumerate(utt_hyps.values()) if index<n_best]
    ...
    if "hyp_score" in requirements:
        hyp_score = [
            [hyp["score"] for index, hyp in enumerate(utt_hyps.values()) if index<n_best]
            for utt_hyps in all_hyps
        ]
    ...
    if "hyp_cer" in requirements:
        hyp_cer = [
            [hyp["cer"] for index, hyp in enumerate(utt_hyps.values()) if index<n_best]
            for utt_hyps in all_hyps
        ]
    ...
    if "alignment" in requirements:
        alignment = [
            [hyp["alignment"] for index, hyp in enumerate(utt_hyps.values()) if index<n_best]
            for utt_hyps in all_hyps
        ]
